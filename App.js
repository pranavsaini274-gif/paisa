...
// Gemini API Key
const GEMINI_API_KEY = 'AIzaSyDQZ9UPe64K_gMeaZNx1hAgIZWkvbhgDYA';
...
function parseSMS(body, date) {
  ...
  const amountMatch = body.match(/(?:rs\.?|inr|₹)\s*([0-9,]+(?:\.[0-9]{1,2})?)/i)
    || body.match(/([0-9,]+(?:\.[0-9]{1,2})?)\s*(?:rs\.?|inr|₹)/i);
  ...
  const isDebit = /debited|paid|sent|spent|withdrawn|deducted|payment|purchase|used for/i.test(low);
  const isCredit = /credited|received|refund|cashback/i.test(low);
  if (!isDebit && !isCredit) return null;
  const type = isDebit ? 'debit' : 'credit';

  // Determine method
  let method = 'upi';
  if (/atm|withdrawn|cash/i.test(low)) method = 'atm';
  else if (/card|visa|master|rupay/i.test(low)) method = 'card';
  else if (/neft|imps|rtgs/i.test(low)) method = 'neft';
  else if (/upi/i.test(low)) method = 'upi';

  // Extract merchant - try multiple patterns
  let merchant = '';
  const merchantPatterns = [
    /(?:to|at|paid to|sent to|trf to|transfer to)\s+([A-Za-z][A-Za-z0-9 &\-\.]{1,30}?)(?:\s+on|\s+via|\s+ref|\s+upi|\.|,|\n|$)/i,
    /(?:at|merchant)\s*:\s*([A-Za-z][A-Za-z0-9 &\-\.]{1,30}?)(?:\s|,|\.|$)/i,
    /([A-Za-z][A-Za-z0-9 &\-\.]{2,25}?)\s+(?:has been paid|was paid)/i,
    /vpa\s*[:\-]?\s*([a-zA-Z0-9._]+)@/i,
    /UPI\s*[:\-]?\s*([a-zA-Z0-9._]+)@/i,
  ];
  ...
  const balMatch = body.match(/(?:bal(?:ance)?|avl|available)\s*(?:rs\.?|inr|₹)?\s*([0-9,]+(?:\.[0-9]+)?)/i);
  const refMatch = body.match(/(?:ref(?:no|\.?\s*no\.?)?|txn\s*(?:id|no)?|upi\s*ref)\s*[:\-#]?\s*([A-Z0-9]{6,25})/i);
...
async function geminiCategorize(merchant, amount, rawSMS, geminiKey) {
  const key = (geminiKey && geminiKey.trim()) || GEMINI_API_KEY;
  if (!key) return null;
  try {
    const catList = CATEGORIES.map(c => c.id + ': ' + c.name).join(', ');
    const prompt = 'Categorize this Indian bank transaction. Merchant: ' + merchant +
      '. Amount: Rs.' + amount +
      '. SMS snippet: ' + (rawSMS || '').slice(0, 200) +
      '. Choose ONE category id from: ' + catList +
      '. Reply ONLY with a JSON object like {"catId":"groceries"} and nothing else.';
    ...
    if (d.error) { console.log('Gemini error:', d.error.message); return null; }
    const txt = d.candidates?.[0]?.content?.parts?.[0]?.text || '';
    const clean = txt.replace(/```json|```/g, '').trim();
    const jsonMatch = clean.match(/\{[^}]+\}/);
    if (jsonMatch) {
      const parsed = JSON.parse(jsonMatch[0]);
      const validCat = CATEGORIES.find(c => c.id === parsed.catId);
      if (validCat) return { cat: parsed.catId, confidence: 0.85 };
    }
  } catch (e) {
    console.log('geminiCategorize error:', e.message);
  }
  return null;
}
  ...
  const handleSync = async () => {
    ...
    try {
      if (Platform.OS !== 'android') {
        Alert.alert('Android Only', 'SMS reading only works on Android.');
        setSyncing(false);
        return;
      }
      ...
      const granted = await PermissionsAndroid.request(
        PermissionsAndroid.PERMISSIONS.READ_SMS,
        {
          ...
          message: 'Paisa needs to read your SMS to find bank transactions.',
      ...
      // Read SMS - last 6 months
      const minDate = Date.now() - 180 * 24 * 60 * 60 * 1000;
      const filter = JSON.stringify({ box: 'inbox', minDate, maxCount: 1000 });

      SmsAndroid.list(
        filter,
        (fail) => {
          Alert.alert('Could not read SMS', String(fail));
          setSyncing(false);
        },
        async (count, smsList) => {
          try {
            const messages = JSON.parse(smsList);
            const gKey = await AsyncStorage.getItem(KEYS.GEMINI_KEY);
            const activeKey = (gKey && gKey.trim()) || GEMINI_API_KEY;
            let added = 0;
            let skipped = 0;
            ...
            for (const msg of messages) {
              ...
              // Parse - our new parser is much more permissive
              const msgDate = msg.date
                ? new Date(Number(msg.date)).toISOString()
                : new Date().toISOString();

              const txn = parseSMS(body, msgDate);
              if (!txn) { skipped++; continue; }

              // Dedup by ref number
              if (txn.ref && _txns.some(t => t.ref === txn.ref)) continue;

              // Categorize
              let catResult = DB.applyRules(txn.merchant, body);
              let catId = catResult ? (catResult.cat || catResult) : null;

              if (!catId) {
                const kwResult = keywordCategorize(txn.merchant, body);
                if (kwResult && kwResult.cat !== 'other') catId = kwResult.cat;
              }

              if (!catId && activeKey) {
                try {
                  const gr = await geminiCategorize(txn.merchant, txn.amount, body, activeKey);
                  if (gr) catId = gr.cat || gr;
                } catch {}
              }
            ...
            Alert.alert(
              'Sync Complete',
              added > 0
                ? added + ' transactions imported!\n(' + skipped + ' messages were not bank transactions)'
                : 'No new transactions found.\n\nChecked ' + messages.length + ' messages, ' + skipped + ' were not bank transactions.\n\nTip: Make sure bank SMS are in your main inbox, not spam folder.'
            );
          } catch (e) {
            Alert.alert('Error processing SMS', e.message);
            setSyncing(false);
          }
        }
      );
...
// ============================================================
// TROUBLESHOOTER SCREEN (Gemini AI Chat)
// ============================================================
function TroubleshooterScreen() {
  const [messages, setMessages] = useState([
    { role: 'assistant', text: "Hi! I'm your Paisa troubleshooter 🛠️\n\nI can help you fix issues like:\n• SMS not being read\n• Transactions not showing\n• Permission problems\n• Wrong categories\n\nWhat's going wrong?" }
  ]);
  ...
  useEffect(() => {
    AsyncStorage.getItem(KEYS.GEMINI_KEY).then(k => setGeminiKey(k || GEMINI_API_KEY));
  }, []);

  const systemPrompt = `You are a helpful troubleshooter for "Paisa", an Android expense tracker app built with React Native and Expo.
The app reads bank SMS messages to auto-track expenses. Common issues:
1. SMS not reading: User needs to grant SMS permission in Android Settings > Apps > Paisa > Permissions > SMS. Also bank SMS must be in inbox not spam. The app reads from "bank, hdfc, sbi, icici, axis, kotak, idbi, pnb, boi, canara, ubi, upi, gpay, phonepe, paytm, amazon, flipkart, razorpay" senders.
2. Permission denied: Go to Settings > Apps > Paisa > Permissions > SMS and toggle it on.
3. Transactions not showing after sync: Check if bank SMS is in inbox not spam folder. Try moving one to inbox.
4. Wrong categories: Use "Teach Paisa" feature in More menu to train it.
5. App crashes: Usually a permissions issue.
Be concise, friendly, and give step-by-step fixes. Use simple language since the user may not be technical.`;
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      const reply = data?.candidates?.[0]?.content?.parts?.[0]?.text || 'Sorry, I could not get a response. Check your API key.';
      setMessages(prev => [...prev, { role: 'assistant', text: reply }]);
    } catch (e) {
      setMessages(prev => [...prev, { role: 'assistant', text: 'Network error. Please check your internet connection.' }]);
    } finally {
      setLoading(false);
  ...
  return (
    <KeyboardAvoidingView style={{ flex: 1, backgroundColor: C.bg }} behavior="padding">
      ...
      >
        {!geminiKey && (
          <View style={{ backgroundColor: C.amberLight, borderRadius: 12, padding: 14, marginBottom: 16 }}>
            <Text style={{ color: C.amber, fontWeight: '700', marginBottom: 4 }}>⚠️ Gemini API Key Required</Text>
            <Text style={{ color: C.text, fontSize: 13 }}>Go to More → Settings → scroll down to the Tech section and paste your Gemini API key to use the troubleshooter.</Text>
          </View>
        )}
        {messages.map((msg, i) => (
          <View key={i} style={{
            alignSelf: msg.role === 'user' ? 'flex-end' : 'flex-start',
            backgroundColor: msg.role === 'user' ? C.primary : C.bgCard,
            borderRadius: 16,
            ...
            <Text style={{ color: msg.role === 'user' ? C.white : C.text, fontSize: 14, lineHeight: 20 }}>
              {msg.text}
            </Text>
          </View>
        ))}
        {loading && (
          <View style={{ alignSelf: 'flex-start', backgroundColor: C.bgCard, borderRadius: 16, padding: 12, marginBottom: 10 }}>
            <ActivityIndicator size="small" color={C.primary} />
          </View>
        )}
      </ScrollView>
      <View style={{ flexDirection: 'row', padding: 12, backgroundColor: C.bgCard, borderTopWidth: 0.5, borderTopColor: C.border, gap: 10 }}>
        <TextInput
          style={{ flex: 1, backgroundColor: C.bgMuted, borderRadius: 20, paddingHorizontal: 16, paddingVertical: 10, fontSize: 15, color: C.text }}
        ...
        <TouchableOpacity
          ...
          style={{ width: 44, height: 44, borderRadius: 22, backgroundColor: input.trim() ? C.primary : C.bgMuted, alignItems: 'center', justifyContent: 'center' }}
        >
          <Ionicons name="send" size={18} color={input.trim() ? C.white : C.textTertiary} />
        </TouchableOpacity>
      </View>
    </KeyboardAvoidingView>
  );
  ...
  return (
    <ScrollView style={{ flex:1, backgroundColor: C.bg }} contentContainerStyle={{ padding: 20 }}>
      {[
        ...
        { icon: '🛠️', title: 'Troubleshooter', sub: 'Fix issues with AI help', screen: 'Troubleshooter' },
      ].map(item => (
    ...
    <SafeAreaProvider>
      <NavigationContainer>
        <Stack.Navigator screenOptions={{
          ...
          <Stack.Screen name="Troubleshooter" component={TroubleshooterScreen} options={{ title: '🛠️ Troubleshooter' }} />
        </Stack.Navigator>
      </NavigationContainer>
    </SafeAreaProvider>
  );
