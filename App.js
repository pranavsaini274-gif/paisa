      ...
      SmsAndroid.list(
        ...
        (count, smsList) => {
          try {
            ...
            const bankSenders = /bank|hdfc|sbi|icici|axis|kotak|idbi|pnb|boi|canara|ubi|upi|gpay|phonepe|paytm|amazon|flipkart|juspay|razorpay|neft|imps/i;
            let added = 0;
            const geminiKey = null; // will be loaded from storage
            AsyncStorage.getItem(KEYS.GEMINI_KEY).then(async (gKey) => {
              for (const msg of messages) {
                if (!bankSenders.test(msg.address || '')) continue;
                const txn = parseSMS(msg.body, msg.date);
                if (!txn) continue;
                // Skip duplicates by ref
                ...
                if (rule) catId = rule;
                if (!catId && gKey) catId = await geminiCategorize(txn.merchant, txn.amount, txn.rawSMS, gKey);
                await DB.addTxn({ ...txn, catId: catId || 'other', needsReview: !catId });
                added++;
              }
              load();
              setSyncing(false);
...
function TroubleshooterScreen() {
  ...
  useEffect(() => {
    AsyncStorage.getItem(KEYS.GEMINI_KEY).then(k => setGeminiKey(k || ''));
  }, []);
...
Be concise, friendly, and give step-by-step fixes. Use simple language since the user may not be technical.`;
  ...
  const sendMessage = async () => {
    ...
    if (!geminiKey) {
      Alert.alert('API Key Needed', 'Please add your Gemini API key in Settings > Tech section first.');
      return;
    }
