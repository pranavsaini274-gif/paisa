  ...
  const handleSync = async () => {
    ...
    try {
      if (Platform.OS !== 'android') {
        Alert.alert('Android Only', 'SMS reading is only available on Android.');
        setSyncing(false);
        return;
      }
      ...
      if (granted !== PermissionsAndroid.RESULTS.GRANTED) {
        Alert.alert('Permission Denied', 'Please allow SMS access in Settings > Apps > Paisa > Permissions > SMS.');
        setSyncing(false);
        return;
      }
      // Read last 90 days of SMS
      const minDate = Date.now() - 90 * 24 * 60 * 60 * 1000;
      SmsAndroid.list(
        JSON.stringify({ box: 'inbox', minDate, maxCount: 500 }),
        (fail) => {
          Alert.alert('SMS Read Failed', `Could not read SMS: ${fail}\n\nMake sure SMS permission is granted in phone Settings.`);
          setSyncing(false);
        },
        (count, smsList) => {
          try {
            const messages = JSON.parse(smsList);
            const bankSenders = /bank|hdfc|sbi|icici|axis|kotak|idbi|pnb|boi|canara|ubi|upi|gpay|phonepe|paytm|amazon|flipkart|juspay|razorpay|neft|imps|credit|debit|rupay|visa|mastercard|wallet|loan|emi|mobile|VM-|BP-|AD-/i;
            let added = 0;
            AsyncStorage.getItem(KEYS.GEMINI_KEY).then(async (gKey) => {
              const activeKey = gKey || GEMINI_API_KEY;
              for (const msg of messages) {
                const sender = (msg.address || '').toLowerCase();
                const body = msg.body || '';
                // Accept if sender looks like a bank/service OR body has transaction keywords
                const isBankSender = bankSenders.test(sender);
                const hasTransactionKeyword = /debited|credited|spent|withdrawn|paid|transfer|upi|inr|rs\.|rupees/i.test(body);
                if (!isBankSender && !hasTransactionKeyword) continue;
                // Convert ms timestamp to ISO string
                const msgDate = msg.date ? new Date(parseInt(msg.date)).toISOString() : new Date().toISOString();
                const txn = parseSMS(body, msgDate);
                if (!txn) continue;
                // Skip duplicates
                const existing = _txns.find(t => t.ref && t.ref === txn.ref);
                if (existing) continue;
                // Also skip by amount+date combo to catch duplicates without ref
                const dupByAmtDate = _txns.find(t => t.amount === txn.amount && t.date && msgDate && t.date.slice(0,10) === msgDate.slice(0,10));
                if (dupByAmtDate) continue;
                // Auto categorize
                let catId = keywordCategorize(txn.merchant, txn.rawSMS);
                const rule = DB.applyRules(txn.merchant, txn.rawSMS);
                if (rule) catId = rule.cat || rule;
                if (!catId || catId === 'other') {
                  const geminiResult = await geminiCategorize(txn.merchant, txn.amount, txn.rawSMS, activeKey);
                  if (geminiResult) catId = geminiResult.cat || geminiResult;
                }
                await DB.addTxn({ ...txn, catId: catId || 'other', needsReview: !catId || catId === 'other' });
                added++;
              }
              load();
              setSyncing(false);
              Alert.alert('SMS Sync Done', added > 0 ? 'Found ' + added + ' new bank transactions from last 90 days!' : 'No new bank transactions found. Make sure your bank SMS arrives in your inbox (not spam).');
            });
          } catch (parseErr) {
            Alert.alert('Parse Error', parseErr.message);
            setSyncing(false);
          }
        }
      );
